...
/*
  ...
  Revision History:
  ~~~~~~~~~~~~~~~~~
  21.01.22:  - Added the possibility to use the GPIO Pins as send / receive status flags
  22.01.22:  - Changed readMsg() to get the correct receive sequence if the bus load is high
*/
...
#if CAN_STATUS_LED
    StatusLED_Mode = 0;                                                                                       // 23.10.17: Hardi
    NextStatusLED_ChangeR = 0;
    NextStatusLED_ChangeT = 0;
#endif

#if CORRECT_RX_SEQUENZ                                                                                        // 22.01.22:  Hardi
    ReadNext = 0;
#endif
...
#if CAN_STATUS_LED
    if (StatusLED_Mode && millis() > NextStatusLED_ChangeR)                                                   // 23.10.17: Hardi
       {
       NextStatusLED_ChangeR = millis() + 50;
       StatusLED_Mode ^= 0x20; // Invert bit 5 (B1BFS: RX1BFPin State bit)
       Write_MCP_BFPCTRL(StatusLED_Mode);
       }
#endif
}
...
#if CAN_STATUS_LED
    if (StatusLED_Mode && millis() > NextStatusLED_ChangeT)                                                   // 23.10.17: Hardi
       {
       NextStatusLED_ChangeT = millis() + 50;
       StatusLED_Mode ^= 0x10; // Invert bit 5 (B0BFS: RX1BFPin State bit)
       Write_MCP_BFPCTRL(StatusLED_Mode);
       }
#endif
    return CAN_OK;
...
*********************************************************************************************************/
/*
 Problem beim empfangen von CAN Messages mit bei hoher CAN Bus Last
 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 Wenn mehrere Messages so schnell hintereinander ankommen, dass die Aplikation die Daten nicht schnell
 genug auslesen kann, dann werden evtl. die Messages in falscher Reihenfolge ausgelesen.
 Das habe ich beobachtet beim auslesen der MS2 Konfiguration. Hier sendet die MS2 schnell eine "Textdatei"
 Dabei kommt es manchmal vor, dass die Reihenfolge der Daten durcheinander gerät.
 Laut dem Flow chart Figure 4-3 in MCP2515.pdf auf Seite 26 werden die Empfangenen CAN NAchrichten
 zunächst in RXB0 gespeichert. Wenn dieser Puffer voll ist, dann wird eine weitere Message in RXB1
 abgelegt. Eine dritte Nachricht wird, falls RXB0 inzwischen gelesen wurde wieder in RXB0 abgelegt.
 Das führt dann beim auslesen mit dem bestehenden readMsg() zu einer falschen Sequenz ;-(
  Msg1 -> RXB0
  Msg2 -> RXB1
  Auslesen RXB0 (Msg1)
  Msg3 -> RXB0
 ...
 Darum habe ich das Programm angepasst:
  Msg1 -> RXB0
  Msg2 -> RXB1
  Auslesen RXB0 (Msg1)
  Msg3 -> RXB0
  Auslesen RXB1 (Msg2) *       wenn in beiden Register Daten vorhanden sind wird zuerst RXB1 gelesen
  Auslesen RXB0 (Msg3)

 Was passiert wenn eine weitere Message dazu kommt?
  Msg1 -> RXB0
  Msg2 -> RXB1
  Auslesen RXB0 (Msg1)
  Msg3 -> RXB0
  Auslesen RXB1 (Msg2)
* Msg4 -> RXB1
  Auslesen RXB1 (Msg4)         Da wieder in beiden Registern Daten liegen wird nach dem neuen Algo jetzt die falsche Nachricht gelesen ;-(
  Auslesen RXB0 (Msg3)

 => So geht es nicht ;-(

 Neuer Ansatz:
 Wenn in beiden Registern Daten vorhanden sind, dann wird abwechselnd aus dem Register 1 und 0 gelesen.
 Es wird eine Variable ReadNext Eingeführt welche 0 oder 1 enthält.
   ReadNext = 0
   if (Daten in beiden Registern)
        {
        Lese Register ReadNext
        ReadNext = !ReadNext     Beim nächsten mal das andere Register lesen
        }
   else {
        if (Daten in Reg 0)
           {
  ...
  Msg1 -> RXB0
  Msg2 -> RXB1
  Auslesen RXB0 (Msg1)    Weil ReadNext == 0 wird RXB0 gelesen => ReadNext = 1
  Msg3 -> RXB0
  Auslesen RXB1 (Msg2)    Weil ReadNext == 1 wird RXB1 gelesen => ReadNext = 0
  Msg4 -> RXB1
  Auslesen RXB0 (Msg3)    Weil ReadNext == 0 wird RXB0 gelesen => ReadNext = 1
  Msg5 -> RXB0
  Auslesen RXB1 (Msg4)    Weil ReadNext == 1 wird RXB1 gelesen => ReadNext = 0
  Auslesen RXB0 (Msg5)

  So sollte es funktionieren.

  !!! Aber Tests haben gezeigt, das noch nicht so richtig geht. Es verschlechtert die Situation eher !!!
  => Darum ist es wieder abgeschaltet

*/
INT8U MCP_CAN::readMsg()
{
 #if CORRECT_RX_SEQUENZ
    INT8U stat = mcp2515_readStatus();
    if ( (stat & (MCP_STAT_RX0IF | MCP_STAT_RX1IF)) == (MCP_STAT_RX0IF | MCP_STAT_RX1IF)) // Messages in both registers
         {
         // Read the register which is defined by the prior defined ReadNext
         }
    else {
         if (      stat & MCP_STAT_RX0IF ) ReadNext = 0;   // Msg in Buffer 0
         else if ( stat & MCP_STAT_RX1IF ) ReadNext = 1;   // Msg in Buffer 1
         else return CAN_NOMSG;                            // No messages available => returm
         }

    // This part is only reached if messages are available
    if ( ReadNext == 0) //    read message form Buffer 0
         {
         ReadNext = 1;
         mcp2515_read_canMsg( MCP_RXBUF_0);
         mcp2515_modifyRegister(MCP_CANINTF, MCP_RX0IF, 0);
         }
    else { // ReadNext == 1   read message form Buffer 1
         ReadNext = 0;
         mcp2515_read_canMsg( MCP_RXBUF_1);
         mcp2515_modifyRegister(MCP_CANINTF, MCP_RX1IF, 0);
         }

    if ( (stat & (MCP_STAT_RX0IF | MCP_STAT_RX1IF)) == 0) // both registers empty                             // 23.01.22:
       ReadNext = 0;
    return CAN_OK;

 #else // Old without CORRECT_RX_SEQUENZ
...
void MCP_CAN::Write_MCP_BFPCTRL(INT8U Val)                                                                     // 22.10.17:
//----------------------------------------
// Write the BFPCTRL register which controlls the RX0BF and RX1BF pins
//
...
//                       0 = Digital Output mode
//
// Example:       its:76543210
// Write_MCP_BFPCTRL(B00101000); Use RX1BF as output and enable it
// Write_MCP_BFPCTRL(B00001000); Use RX1BF as output and disable it
// Write_MCP_BFPCTRL(B00111100); Use RX0BF and RX1BF as output and enable it
// Write_MCP_BFPCTRL(B00001100); Use RX1BF and RX1BF as output and disable it
//
// Write_MCP_BFPCTRL(B00001111); Use RX1BF and RX1BF as buffer full indikator
{
  mcp2515_setRegister(MCP_BFPCTRL, Val);
}

#if CAN_STATUS_LED
//----------------------------------
void MCP_CAN::EnableStatusLEDs(void)                                                                           // 23.10.17:
//----------------------------------
// Enables the Status LEDs which are connected to RX1BF/RX0BF pins (Receive/Transmit).
// The function must be called after the initialization with begin()
//
{
   StatusLED_Mode = STATUS_LED_OFF;
   Write_MCP_BFPCTRL(StatusLED_Mode); // Activate the output and disable the LEDs
}
#endif
