...
#ifndef CAN_STATUS_LED
   #define CAN_STATUS_LED  1                                                                                  // 21.01.22:
#endif

#ifndef CORRECT_RX_SEQUENZ
  #define CORRECT_RX_SEQUENZ  0                                                                               // 22.01.22:
  // So wie es aussieht funktioniert das noch nicht so richtig. Es verschlechtert die Situation eher
#endif


#include "mcp_can_dfs_nd.h"
...
#if CAN_STATUS_LED
     INT8U  StatusLED_Mode;                                             //  Mode of the status LEDs            // 23.10.17:
     uint32_t NextStatusLED_ChangeR;                                    //  0    = disabled
     uint32_t NextStatusLED_ChangeT;                                    //  0x0C = enabled
#endif                                                                  //  0x20: Flag used for RX1BF pin (Receive)
                                                                        //  0x10: Flag used for RX0BF pin (Transmit)
...
public:
    ...
    void  Write_MCP_BFPCTRL(INT8U Val);                                                         // Write the BFPCTRL register which controls the RX0BF and RX1BF pins      // 22.10.17: Hardi
                                                                                                // The function could be used to enable/disable the RX0BF and RX1BF pins
                                                                                                // or enable the automatic pin change function if the rexeive buffers
                                                                                                // 0/1 are filled which could be used to generate interrupts.
#if CAN_STATUS_LED
    void  EnableStatusLEDs(void);                                                               // Enable the status LEDs which are connected to the RX0BF and RX1BF pins   // 23.10.17: Hardi
#endif                                                                                          // The LEDs toggle there state with each received/transmitted CAN message.
                                                                                                // The LEDs should be connected to +5V(3.3V). If they are connected to
                                                                                                // GND adapt STATUS_LED_OFF in "mcp_can_dfs.h"
